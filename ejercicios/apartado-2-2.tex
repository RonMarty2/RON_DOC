...
\minihistoria{Dónde vive cada una en tu trabajo}{%
  \textbf{Clásica} — asignas 40 participantes al azar a tratamiento o control.
  La probabilidad de que a alguien le toque tratamiento es exactamente
  $1/2$, y lo sabes \emph{antes} de empezar: la simetría la fabricaste tú con
...
\ejercicio{0}{¿Cuáles de estos números pueden ser una probabilidad? Marca los
  que no y explica por qué.
...
\solucion{%
  ...
  falta revisar nada más — ya sabes que hay un error.}
\enclase{Pídeles que digan en voz alta qué significaría una probabilidad de
...
\ejercicio{1}{Un estudio asigna al azar 40 participantes: 20 a tratamiento y
  20 a control.
  \textbf{Antes de calcular, escribe qué probabilidad crees que tiene un
  participante de quedar en tratamiento:} \underline{\hspace{2cm}}
  \par\vspace{0.2em}
  Ahora calcúlala. ¿Qué tipo de probabilidad usaste y por qué te alcanzó con
  el enunciado, sin observar ningún dato?}{Predice primero}
\renglones[4]
...
\ejercicio{1}{De 200 pacientes que iniciaron terapia en el año, 46 abandonaron
  antes de la sexta sesión. Calcula la probabilidad de que un paciente elegido
...
\ejercicio{2}{Dos situaciones que dan el \emph{mismo número} y no son la misma
  cosa. Calcula las dos y explica en qué se diferencian.
...
\solucion{%
  ...
  repites el estudio, sigue siendo $0{,}5$ exactamente.
  \par
  (b) Se supo \textbf{después} de observar 100 casos. Si sigues a los próximos
  100 pacientes, casi seguro no da 50 de nuevo — puede dar 47 o 54.
  \par
  \textbf{Por qué importa}: sobre (a) puedes apostar sin datos; sobre (b) tienes
  que preguntarte cuántos casos observaste. Cincuenta de 100 y 5 de 10 dan el
  mismo $0{,}5$ y no merecen la misma confianza.}
\enclase{Pregunta cuál de las dos cambiaría si repitieran el estudio. Es la
...
\ejercicio{3}{De las 200 fichas del servicio, 43 llegan al corte de 10 o más y
  ...
  azar.
  \begin{enumerate}[label=(\alph*),topsep=2pt,itemsep=1pt]
    \item Calcula $P(\text{dar positivo})$ y $P(\text{no dar positivo})$.
    \item Calcula $P(\text{tener el diagnóstico})$. Ese número tiene nombre
      propio: ¿cuál?
...
\solucion{%
  ...
  proporción de la población tiene la condición. Guárdalo — en 2.6 va a ser la
...
\ejercicio{3}{El servicio quiere estimar la probabilidad de que un estudiante
  ...
  entrevista. \textbf{¿Alcanza para responder?} Si sí, explica cómo; si no,
  di qué falta.}{¿Alcanzan los datos?}
\renglones[4]
...
\ejercicio{4}{Clasifica cada afirmación como \emph{clásica}, \emph{frecuentista}
  o \emph{subjetiva}, y justifica en una línea.
  \begin{enumerate}[label=(\alph*),topsep=2pt,itemsep=1pt]
  ...
  \end{enumerate}
  Después contesta: ¿cuál de las cuatro cambiaría si el estudio se repitiera
  con otras personas?}{}
\renglones[6]
...
quien entendió. Déjala para el cierre de la clase.}
